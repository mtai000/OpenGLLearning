...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    if (dotProduct(ray.direction, normal) > 0)
        return inter;
    double u, v, t_tmp = 0;
    Vector3f pvec = crossProduct(ray.direction, e2);    //S1 = crossProduct(dir,e2)
    double det = dotProduct(e1, pvec);                  //det = doctProduct(s1,e1)
    if (fabs(det) < EPSILON)
        return inter;

    double det_inv = 1. / det;                          //det_inv = 1/det
    Vector3f tvec = ray.origin - v0;                    //S = orig - v0
    u = dotProduct(tvec, pvec) * det_inv;               //u = dotProduct(s,s1) / det    -> b1
    if (u < 0 || u > 1)
        return inter;
    Vector3f qvec = crossProduct(tvec, e1);             //S2 = crossProduct(s,e1)
    v = dotProduct(ray.direction, qvec) * det_inv;      //v = dotProduct(dir,s2) / det   -> b2
    if (v < 0 || u + v > 1)
        return inter;
    t_tmp = dotProduct(e2, qvec) * det_inv;             //t_tmp = dotProduct(e2,s2) / det -> t

    // TODO find ray triangle intersection
    if(t_tmp >= 0.0 && u >= 0.0 && v >= 0.0 && u-v <= 1.0)
    {
        inter.happened = true;
        inter.distance = t_tmp;    
        inter.normal = normal;      //法线
        inter.m = m;                //材料
        inter.obj=this;
        inter.coords = ray(t_tmp);  //光传播t单位  o+td;
        return inter;               //光线与此三角相交
    }
    return inter;
}
