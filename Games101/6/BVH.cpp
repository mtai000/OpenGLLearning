...
BVHBuildNode* BVHAccel::SAH_recursiveBuild(std::vector<Object*> objects)
{
    ...
    if (objects.size() == 1) {
        ...
        for(int i = 0; i < objects.size() - 1;i++)
        {
            ...
            float t_cost = (i + 1) * a.SurfaceArea()/bounds.SurfaceArea() + (objects.size() - i - 1) * b.SurfaceArea()/bounds.SurfaceArea();
            if(i == 0) cost = t_cost;
            if(t_cost < cost)
            {
                spliting = objects.begin() + i;
                cost = t_cost;
            }
        }

        auto leftshapes = std::vector<Object*>(beginning, spliting);
        auto rightshapes = std::vector<Object*>(spliting, ending);
